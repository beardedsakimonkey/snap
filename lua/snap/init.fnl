...
(tset M :config config)

(fn M.map [key run opts]
...
(tset M :continue_value {:continue true})

(fn M.continue [on-cancel]
  ...
  (coroutine.yield M.continue_value on-cancel))

(fn M.resume [thread request value]
  ...
  (let [(_ result) (coroutine.resume thread request value)]
    (if
      ...
      (= (type result) :function) (M.resume thread request (M.sync result))
      ;; If we aren't canceling then return result
      result)))

(fn M.consume [producer request]
  "Returns an iterator that consumes a producer"
  (local producer (M.get_producer producer))
...
(fn M.with_meta [result field value]
  ...
  (let [meta-result (M.meta_result result)]
    (tset meta-result field value)
    meta-result))

(fn M.with_metas [result metas]
...
(fn M.run [config]
  ...
  (assertfunction
    (M.get_producer config.producer) "snap.run 'producer' must be a function or a table with a default function")
  ...
  (local layout (or config.layout (. (M.get :layout) :centered)))

  ;; Default to loading creator
  (local loading (or config.loading (M.get :loading)))
  ...
  (fn on-update [filter]
    ...
    (local config {:producer (M.get_producer config.producer) : request})
    ...
    (fn config.on-end []
      (if
        ...
        (M.has_meta (tbl.first results) :score)
    ...
    (fn config.on-value [value]
      ...
      (when (> (length value) 0)
        ...
        ;; as early as we can
        (when (not (M.has_meta (tbl.first results) :score))
          ;; Early write
